...
namespace BFPL.STEPIK
{
    public class MainMenu
    {
            ...
            while (true)
            {
                ...
                switch (shosenPath)
                {
                    ...
                    case "3 2":
                        {
                            Console.WriteLine($"You chose {shosenPath} path.\n");
                            GeniusIdiotV2.StartGeniusIdiotV2Test();
                            PrintBFPL_STEPIK_MainMenu();
                            break;
                        }
                    case "menu":
        ...
        private static void RunFraction()
        {
            ...
            //Fraction last = new Fraction(3, 0); // Выбрасывает исключение из-за деления на ноль

            PrintBFPL_STEPIK_MainMenu();
        }

        // Методы для обработки пунктов меню
        public static void RunXesAndZeros()
        {
            XesAndZeros.GoXesAndZeros();
            PrintBFPL_STEPIK_MainMenu();
        }

        public static void RunPRGSW()
        {
